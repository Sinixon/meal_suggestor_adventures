...
class OrderService(object):
    ...
    def make_order(self, max_menus_recommended, points, chosen_meals, modulo_check, menu_previously_chosen=None, ask_again=False):
        """
        ...
        if option == 'q':
            ...
            print("Total chosen meals: " + str(chosen_meals))
            if chosen_meals == modulo_check*10:
                print("Congrats! You have reached a milestone of " + str(modulo_check*10) + " total chosen meals!\n")
                print("You will receive an extra 50 points.")
                points += 50
                print("\n\nCurrent points: " + str(points))
                modulo_check += 1
            menu_selected = self.menus_recommended[int(option)]
            self.menus_ordered.append(menu_selected)
            self.make_order(max_menus_recommended, points, chosen_meals, modulo_check, menu_previously_chosen=menu_selected)

        else:

            # Re-select an option
            self.make_order(max_menus_recommended, points, chosen_meals, modulo_check, ask_again=True)
